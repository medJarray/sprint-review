/**
 * ╔══════════════════════════════════════════════════════════╗
 * ║   Script de Seed — Sprint Review #24                    ║
 * ║   Insère les données du Sprint 24 dans MongoDB          ║
 * ║   via les 11 collections du backend NestJS              ║
 * ╚══════════════════════════════════════════════════════════╝
 *
 * Usage:
...
async function put(path: string, data: any): Promise<void> {
  const url = `${BASE_URL}${path}`;
  const res = await fetch(url, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
async function seedPages() {
  ...
  // Utilise le batch replace (PUT /pages/:sprintId/batch)
