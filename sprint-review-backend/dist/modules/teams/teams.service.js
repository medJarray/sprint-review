...
let TeamsService = class TeamsService {
    ...
    async findOne(sprintId, teamId) {
        const team = await this.teamModel.findOne({ sprintId, teamId }).exec();
        if (!team)
            throw new common_1.NotFoundException(`Équipe "${teamId}" introuvable pour le sprint "${sprintId}"`);
        return team;
    }
    async update(sprintId, teamId, dto) {
