"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SprintSchema = exports.Sprint = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
const swagger_1 = require("@nestjs/swagger");
let Sprint = class Sprint extends mongoose_2.Document {
};
exports.Sprint = Sprint;
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Identifiant unique du sprint (frontend)' }),
    (0, mongoose_1.Prop)({ required: true, index: true }),
    __metadata("design:type", String)
], Sprint.prototype, "sprintId", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Nom du sprint' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Sprint.prototype, "name", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Numéro du sprint' }),
    (0, mongoose_1.Prop)({ required: true, index: true }),
    __metadata("design:type", Number)
], Sprint.prototype, "number", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Objectif principal du sprint' }),
    (0, mongoose_1.Prop)({ default: '' }),
    __metadata("design:type", String)
], Sprint.prototype, "goal", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Date de début' }),
    (0, mongoose_1.Prop)({ default: '' }),
    __metadata("design:type", String)
], Sprint.prototype, "startDate", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Date de fin' }),
    (0, mongoose_1.Prop)({ default: '' }),
    __metadata("design:type", String)
], Sprint.prototype, "endDate", void 0);
exports.Sprint = Sprint = __decorate([
    (0, mongoose_1.Schema)({ timestamps: true, collection: 'sprints' })
], Sprint);
exports.SprintSchema = mongoose_1.SchemaFactory.createForClass(Sprint);
exports.SprintSchema.index({ sprintId: 1 }, { unique: true });
//# sourceMappingURL=sprint.schema.js.map